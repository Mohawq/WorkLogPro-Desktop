...
│   ├── auth.js                    # Magic-link + password sign-in/out, session helpers, settings-sheet auth UI wiring
...
- `auth.js` — magic-link and password sign-in/out, session helpers (`getSession()`/`onAuthStateChange()`), and the settings-sheet auth UI (`renderAuthSettings()`, `handleSendMagicLink()`, `handlePasswordAuthSubmit()`, `handleSyncNowClick()`, `handleSignOutClick()`). See section 4L for why both sign-in methods exist.
...
### L. Magic-Link Redirect Handling

Clicking the sign-in email's confirmation link needs to land back in the app with a session — Supabase's default redirect target (`localhost:3000`, a placeholder) doesn't do that on either platform, so both shells define `window.platformAdapter.getAuthRedirectUrl()` and `core/auth.js`'s `signInWithMagicLink()` passes its return value as `emailRedirectTo` in the `signInWithOtp()` call. This is the pattern to follow for any **future** platform-adapter method, not just this one: the capability's *implementation* differs per shell, but the *call site* lives once in `core/`, exactly like `exportPDF` (section 4A).

- `platform-web/index.html`: `() => window.location.origin` — the live origin at runtime, not a hardcoded string, so this works unchanged against production, a Vercel preview deployment, or local `vercel dev`.
- `platform-electron/shell.html`: `() => 'worklogpro://auth'` — Electron has no localhost web server to redirect a browser to, so it uses a custom URL protocol instead (below).

**Required manual step, not something code can do or verify**: Supabase Dashboard → Authentication → URL Configuration → Redirect URLs must explicitly list **both** `https://<the Vercel domain>/**` and `worklogpro://auth/**` (Site URL should be the Vercel domain too). Supabase silently ignores any `emailRedirectTo` that isn't covered by an allow-listed pattern there and falls back to the `localhost:3000` placeholder instead — that's the entire original bug, and the dashboard side of the fix has no code equivalent. Don't assume this is configured; if magic links still land on `localhost:3000` after a deploy, this is the first thing to check, not the code above.

**`platform-electron/main.js`** — the custom `worklogpro://` protocol:

- Registered via `app.setAsDefaultProtocolClient('worklogpro')` every launch inside `whenReady()` (cheap, idempotent). Dev mode (`process.defaultApp` — running via `electron .`, not a packaged `.exe`) needs the exact `process.execPath`/argv invocation passed explicitly, or Windows registers bare `electron.exe` with no arguments and a protocol click just opens a blank Electron window instead of this app.
- `app.requestSingleInstanceLock()` is checked — and on failure, `app.quit()` + an early top-level `return` — before any window/tray setup, at the very top of the file. This is the same failure mode as the tray-icon duplicate-instance incident (section 4H's `initProjectFlow()` writeup is a different bug, but the "don't ever let a second instance touch shared state" lesson is the same one) applied to a new trigger: a protocol click while the app is already running would otherwise launch a second process.
- Three arrival paths for the callback URL, all funneling into `sendAuthCallbackToRenderer()`:
  - `'second-instance'` (Windows/Linux, app already running) — the URL is in the new process's `argv`, which Electron hands to the first instance via this event; the second process exits without ever creating a window.
  - `'open-url'` (macOS) — registered at the top level, not inside `whenReady().then()`, since macOS can fire this before the app finishes launching.
  - `process.argv`, checked once inside `whenReady().then()` — a cold start where the protocol click *is* the launch, so there's no earlier instance for the other two paths to have fired on.
  - If the renderer isn't ready yet when any of these fire (`win` doesn't exist, or its page hasn't finished loading), the URL is held in `pendingAuthUrl` and flushed from `createWindow()`'s `did-finish-load` handler instead of being dropped.

**`platform-electron/preload.js`** — one new bridge, `window.authAPI.onAuthCallback(callback)`, wrapping `ipcRenderer.on('auth-callback', ...)`. Same "exactly one purpose-built method, no generic IPC passthrough" design as `invoiceAPI.exportPDF` (section 4I) — a new IPC need gets its own named method here, never a generic channel.

**`core/auth.js`**: `initAuthUI()` registers `handleElectronAuthCallback` with `window.authAPI.onAuthCallback` when that global exists — a no-op on the web shell, which has no `window.authAPI` at all (a browser's magic-link redirect is an ordinary page load; supabase-js's own `detectSessionInUrl`, on by default, already handles pulling the session out of `window.location.hash` there — no code needed for the web side beyond fixing where it redirects *to*). `handleElectronAuthCallback(url)` parses the `worklogpro://auth#access_token=...&refresh_token=...` fragment and completes the session via `client.auth.setSession({ access_token, refresh_token })` — **not** `getSessionFromUrl()`, which isn't part of the installed `@supabase/supabase-js@2` API; `setSession()` with an explicit token pair is the v2-correct way to establish a session that arrived outside a real browser navigation. Also handles the `#error=...&error_description=...` variant (an expired or already-used link) and a missing-token variant, surfacing either as a message in `authStatusMessage` rather than failing silently. Once `setSession()` succeeds, the existing `onAuthStateChange()` `SIGNED_IN` handler (section 4K) takes over unchanged — same reconcile-then-sync path the web magic-link flow already uses.

**Password sign-in (alongside magic link, not instead of it)**: an installed iOS home-screen PWA gets a storage context fully isolated from regular Safari tabs. Completing a magic link always bounces through Safari at some point (the email app opens the link there), so the resulting session lands in Safari's storage, not the installed app's — the app never sees it, no matter how correct the redirect-URL wiring above is. This is a hard iOS platform constraint, not something fixable in the magic-link flow itself, so password auth exists as a second method that completes entirely in-process with no redirect at all. Desktop keeps magic link as its primary path (no storage-isolation issue there) — password is just an extra option.

- `core/auth.js`'s `signUpWithPassword(email, password)` / `signInWithPassword(email, password)` mirror `signInWithMagicLink()`'s defensive shape (check `initSupabaseClient()` is non-null, throw a clear error otherwise) but call `client.auth.signUp()` / `client.auth.signInWithPassword()` instead. `signUpWithPassword()` returns `{ needsConfirmation, user }` rather than throwing on the common case: whether a session comes back immediately depends on the Supabase dashboard's "Confirm email" setting, which this code doesn't know and doesn't need to — `needsConfirmation: !data.session` self-adapts to either. It also detects Supabase's anti-enumeration behavior (signing up again with an email that already has a *confirmed* account returns success with no error, but an empty `identities` array instead of a new one) and turns that into a real thrown "already exists" error, since otherwise a repeat signup attempt would show "check your email" for a confirmation email that's never coming.
- A session established via `signInWithPassword()`/a confirmed-immediately `signUpWithPassword()` is indistinguishable from a magic-link session to the rest of the app — `onAuthStateChange()`'s `SIGNED_IN` handler, `reconcileLocalWithServer()`, and `runSyncCycle()` (section 4K) all trigger exactly the same way regardless of which method produced the session.
- UI: both shells' Cloud Sync block now has a Magic Link / Password mode toggle (`setAuthMode()`) above two forms that both stay in the DOM (`authMagicLinkForm` / `authPasswordForm`, one hidden at a time) — same two-button-toggle pattern `core/invoicing.js`'s `applyProductModeButtonStyles()` already uses elsewhere. The password form has its own Sign In/Create Account sub-toggle (`setPasswordFormSignUpMode()`/`togglePasswordSignUpMode()`, tracked via a hidden `authPasswordFormMode` input, same convention as `invProductAmountMode`) and a show/hide password button (`togglePasswordVisibility()`). All of it shares the existing `authStatusMessage` element and `friendlyAuthError()` maps the handful of common Supabase auth errors (wrong password, unconfirmed email, already-registered) to clearer text — not an exhaustive error-code table, everything else just falls through to `error.message` as-is.
- `platform-web/index.html`'s copy follows the section's sizing convention (44px tap targets, 16px input font) like every other element added there since; `platform-electron/shell.html`'s copy doesn't need that, same as the rest of its markup.
