...
// A second launch (e.g. double-clicking the app/shortcut again while it's
// already running) just starts a SECOND process, whose only job is to
// focus the first instance's window (via 'second-instance' below) and
// exit — never allowed to open its own window. This used to also matter
// for handing off a worklogpro:// protocol URL (magic-link sign-in,
// removed — see CLAUDE.md section 4L), but kept independent of that: two
// instances writing to the same localStorage/IndexedDB concurrently is
// its own real bug, the same class as the tray-icon duplicate-instance
// incident documented elsewhere in this project. Must run before any
// window/tray setup below.
...
// Fired on the FIRST (already-running) instance whenever a second launch
// is attempted — just brings focus to the existing window instead of
// letting a second one open (see the requestSingleInstanceLock() comment
